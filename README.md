...
### 初めて使う方
1. **Web版を開く**: https://mapcocoro.github.io/kakeiboApp/
2. **支出を記録**: 「入力」タブで日付・カテゴリ・金額を入力
3. **確認**: 「一覧」タブで記録した支出を確認
4. **分析**: 「分析」タブでグラフを確認

### Topixデータを復元したい方
1. **復元ツールを開く**: https://mapcocoro.github.io/kakeiboApp/topix-complete-restore.html
2. **実行**: 「完全復元を実行」ボタンをクリック
3. **確認**: アプリの「推移」タブでTopixを確認

### データをバックアップしたい方
1. 「一覧」タブを開く
2. 「データエクスポート」ボタンをクリック
3. CSVファイルを安全な場所に保存（GoogleドライブやDropboxなど）
...
### Q1. 別のPCやスマホからも同じデータを見れますか？
**A.** データは各デバイスのブラウザに個別保存されるため、自動では同期されません。CSVエクスポート→インポートで手動同期してください。詳しくは「[複数PC・家族で使う方法](#-複数pc家族で使う方法)」をご覧ください。

### Q2. データが消えてしまった場合はどうすればいいですか？
**A.** 定期的にエクスポートしたCSVファイルがあれば、「データインポート」で復元できます。Topixデータは `topix-complete-restore.html` から復元可能です。

### Q3. スマホでも使えますか？
**A.** はい、Web版（https://mapcocoro.github.io/kakeiboApp/）をスマホのブラウザで開けば使えます。ホーム画面に追加すればアプリのように使えます。

### Q4. インターネット接続がないと使えませんか？
**A.** 初回読み込み後はオフラインでも動作しますが、Chart.js（グラフライブラリ）の読み込みにはインターネット接続が必要です。

### Q5. データは安全ですか？どこに保存されますか？
**A.** すべてのデータはあなたのブラウザ内（localStorage）に保存され、外部サーバーには送信されません。ただし、ブラウザのデータ削除で消える可能性があるため、定期的なCSVエクスポートを推奨します。

### Q6. Topixとは何ですか？
**A.** Topix（出来事）は、各月の特記事項やイベントを記録する機能です。「旅行」「車検」「家電購入」など、支出の背景を記録できます。

### Q7. 家族で共有して使いたいのですが？
**A.** メイン管理者を決めて、定期的にCSVエクスポート→家族全員にインポートする方法がおすすめです。詳しくは「[複数PC・家族で使う方法](#-複数pc家族で使う方法)」をご覧ください。

### Q8. データのバックアップはどうすればいいですか？
**A.** 「一覧」タブの「データエクスポート」ボタンでCSVファイルをダウンロードし、Googleドライブなどに保存してください。月1回以上のバックアップを推奨します。
